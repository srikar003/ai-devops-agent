...
async def fetch_pr(state: ReviewState) -> ReviewState:
    ...
    # store useful info in meta via tool_runs? (optional)
    return state
...
def compose_comment_prompt(state: ReviewState) -> str:
    tool_summary = "\n".join(
        f"- {tr.tool}/{tr.action}: {'✅' if tr.ok else '❌'}"
        for tr in state.tool_runs
    )
    findings = [f.model_dump() if hasattr(f, "model_dump") else f for f in state.findings[:20]]
...
async def post_comment(state: ReviewState) -> ReviewState:
    if state.final_comment:
        tr = await gh.post_comment(state.owner, state.repo, state.pr_number, state.final_comment)
        state.tool_runs.append(tr)
    return state
...
def build_graph():
    ...
    # ✅ Simple + reliable sequencing (no duplicate fan-in calls)
    g.add_edge("fetch_pr", "run_ci")
